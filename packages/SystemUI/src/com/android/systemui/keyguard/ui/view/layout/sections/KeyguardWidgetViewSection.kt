...
constructor(
    ...
    override fun addViews(constraintLayout: ConstraintLayout) {
        ...
        widgetView = LayoutInflater.from(context).inflate(
            R.layout.keyguard_clock_widgets,
            constraintLayout,
            false
        ).apply {
        ...
        } as LockScreenWidgets
        
        constraintLayout.addView(widgetView)
    }
        ...
        constraintSet.apply {
            ...
            when {
                constraintSet.getConstraint(R.id.keyguard_info_widgets) != null -> {
                    connect(R.id.keyguard_widgets, ConstraintSet.TOP, R.id.keyguard_info_widgets, ConstraintSet.BOTTOM, 12)
                }
                constraintSet.getConstraint(R.id.keyguard_weather_area) != null -> {
                    connect(R.id.keyguard_widgets, ConstraintSet.TOP, R.id.keyguard_weather_area, ConstraintSet.BOTTOM, 12)
                }
                constraintSet.getConstraint(R.id.clock_ls) != null -> {
                    connect(R.id.keyguard_widgets, ConstraintSet.TOP, R.id.clock_ls, ConstraintSet.BOTTOM, 12)
                }
                constraintSet.getConstraint(R.id.keyguard_slice_view) != null -> {
                    connect(R.id.keyguard_widgets, ConstraintSet.TOP, R.id.keyguard_slice_view, ConstraintSet.BOTTOM, 12)
                }
                else -> {
                    connect(R.id.keyguard_widgets, ConstraintSet.TOP, R.id.lockscreen_clock_view, ConstraintSet.BOTTOM, 12)
                }
            }
            ...
            setMargin(R.id.keyguard_widgets, ConstraintSet.BOTTOM, 6) // 6dp bottom margin for AOD
            setElevation(R.id.keyguard_widgets, 1f)
