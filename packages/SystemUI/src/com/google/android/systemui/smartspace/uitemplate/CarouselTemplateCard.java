...
public class CarouselTemplateCard extends BcSmartspaceCardSecondary {
    ...
    public final boolean setSmartspaceActions(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo) {
        ...
        if (validItems < 4) {
            ...
            for (int i = 0; i < 4; i++) {
                BcSmartspaceTemplateDataUtils.updateVisibility(getChildAt(i), i <= (3 - (4 - validItems)) ? View.VISIBLE : View.GONE);
            }
            ((ConstraintLayout.LayoutParams) ((ConstraintLayout) getChildAt(0)).getLayoutParams()).horizontalChainStyle = (4 - validItems) == 0 ? 1 : 0;
        }
        for (int i = 0; i < validItems; i++) {
            ...
            BcSmartspaceTemplateDataUtils.setText(upperText, ((CarouselTemplateData.CarouselItem) carouselItems.get(i)).getUpperText());
            BcSmartspaceTemplateDataUtils.updateVisibility(upperText, View.VISIBLE);
            BcSmartspaceTemplateDataUtils.setIcon(icon, ((CarouselTemplateData.CarouselItem) carouselItems.get(i)).getImage());
            BcSmartspaceTemplateDataUtils.updateVisibility(icon, View.VISIBLE);
            BcSmartspaceTemplateDataUtils.setText(lowerText, ((CarouselTemplateData.CarouselItem) carouselItems.get(i)).getLowerText());
            BcSmartspaceTemplateDataUtils.updateVisibility(lowerText, View.VISIBLE);
        }
        if (templateData.getCarouselAction() != null) {
            BcSmartSpaceUtil.setOnClickListener(this, target, templateData.getCarouselAction(), eventNotifier, "CarouselTemplateCard", loggingInfo, 0);
        }
